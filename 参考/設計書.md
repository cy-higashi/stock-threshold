...
- **ポータル別（在庫データ用）**
  - `portals[].name`: ポータル名（コマンドラインで渡す「ポータル名のディレクトリ」の**ディレクトリ名**と一致させる。この名前で Excel パスも検索する）
  - **ヘッダーあり**（通常）の場合:
    - `portals[].product_code_column`: 商品コードのカラム名
    - `portals[].stock_column`: 在庫数のカラム名
  - **ヘッダーなし**（例: Choice ポータル）の場合:
    - `portals[].has_header`: `false` を指定
    - `portals[].product_code_column_index`: 商品コードの列番号（0 始まり）
    - `portals[].stock_column_index`: 在庫数の列番号（0 始まり）
  - （任意）`portals[].delimiter`: 区切り文字（未指定時はタブ/カンマを自動判定）
  - （任意）`portals[].encoding`: ファイルエンコーディング（未指定時は utf-8）
...
- **在庫データ**
  ...
  - **ヘッダーあり**のポータル: 商品コード・在庫数を setting.json の `product_code_column` / `stock_column`（カラム名）で指定する。
  - **ヘッダーなし**のポータル（例: Choice）: `has_header` を `false` にし、`product_code_column_index` / `stock_column_index` で列番号（0 始まり）を指定する。
